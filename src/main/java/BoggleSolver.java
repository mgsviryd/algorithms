...
public class BoggleSolver {
    ...
    private static class BoggleTrieST<V> {
         ...
         ****************************************************/
        public void put(final String key, final V val) {
            root = put(root, key, val, 0);
        }

        private Node put(Node x, final String key, final V val, final int d) {
            if (x == null)
                x = new Node();
            if (d == key.length()) {
                x.val = val;
        ...
        public NodeType getNodeType(final String key) {
            Node x = get(root, key, 0);
            if (x == null)
        ...
        private void collect(final Node x, final String key, final Queue<String> queue) {
            if (x == null)
                return;
            if (x.val != null)
                queue.enqueue(key);
        ...
        public void collect(final Node x, final String prefix, final String pat, final Queue<String> q) {
            if (x == null)
                return;
    ...
    private static class BoggleTrie<V> {
    ...
    private void searchWords(final BoggleBoard board, final int i, final int j, final TreeSet<String> words) {
    ...
    private void dfs(final BoggleBoard board, final int i, final int j, final Set<String> words, final boolean[][] visited, String prefix) {
        if (visited[i][j]) {
            return;
        }
