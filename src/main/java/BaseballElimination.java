...
public class BaseballElimination {
    ...
    /**
     * FlowNetwork build for each team(t) separately and team expected as eliminated(tE) not include in flowNetWork.
     * FlowNetwork consists:
     * 1. node Game (link with nodes abstract games where capacity is r[t1][t2])
     * 2. nodes abstract games (f.e. t1-t2 - Matchup) (links with node teams where capacity is PositiveInfinity)
     * 3. nodes team (links with node Team where capacity is w[tE]+r[tE]-w[t])
     * 4. node Team
     * @param teamName
     * @return
     */
    ...
    /**
     * Put in queue strict pairs of teams (Matchups) witch have remaining game
     * @param team - no put in queue pairs with that team
     * @return
     */
    private Queue<Matchup> matchups(int team) {
        Queue<Matchup> matchups = new Queue<>();
